class CommentsController < ApplicationController
  ...
  def create
    comment = current_user.comments.build(comment_params)
      if comment.save
        redirect_to act_path(comment.act), success: t('defalts.message.created', item: Comment.model_name.human)
      else
  ...
  def comment_params
    params.require(:comment).permit(:body).merge(act_id: params[:board_id])
  end
end
